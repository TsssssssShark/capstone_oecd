...
table_output_dir = here::here('table')
...
comparison <- full_join(df_q34, new_data_q34, by = 'country') %>%
  mutate(
    ...
    update_str = replace_na(update_answer, "NA"),
...
comparison_table <- comparison %>%
  select(-status, -original_str, -update_str) %>%
  kable(
    format = "latex",
    booktabs = TRUE,
...
policy_categorisation_table <- create_split_table(
  ...
  export_tex = TRUE,
  tex_filename = 'policy_categorisation_table',
...
summary_available_table <- create_split_table(
  ...
  export_tex = TRUE,
  tex_filename = 'summary_available_table',
