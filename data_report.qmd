...
tryCatch({
  # portalData.js - empty global variable
  js_portal <- paste0(
    "// Global portal data - dynamically populated by DataManager\n",
    "// Generated: ", Sys.Date(), "\n\n",
    "var PORTAL_DATA = [];\n"
  )
  ...
  if (dir.exists(js_output_dir)) {
    writeLines(js_portal, file.path(js_output_dir, "portalData.js"))
  } else {
    stop(glue('JavaScript output directory not found: {js_output_dir}'))
  }
  
  # centralPortalData.js - actual data
  central_portal_map <- central_portal_status %>%
    mutate(
      value = portal_status,
      color = case_when(
  ...
  js_central <- paste0(
    ...
    jsonlite::toJSON(central_portal_map, pretty = TRUE, auto_unbox = TRUE),
    ";\n"
  )
  
  writeLines(js_central, file.path(js_output_dir, "centralPortalData.js"))
...
features_independent_plot <- ggplot(features_independent_nona, 
  ...
  # Mean label for "no" group - y position at mean value
  annotate(
    "text",
    x = 0.65,
  ...
  # Mean label for "yes" group - y position at mean value
  annotate(
    "text",
    x = 1.35,
    y = means_df$mean_value[means_df$independent_website == "yes"],
    label = paste0("Mean: ", 
                   round(means_df$mean_value[means_df$independent_website == "yes"], 2)),
    size = 4,
    hjust = -0.2
  ) +
...
# Calculate means for each group
means_portal_df <- index_portal_df %>%
  group_by(has_portal) %>%
  summarise(
    mean_value = mean(index_overall_no_portal, na.rm = TRUE),
    .groups = 'drop'
  )
...
index_portal_plot <- ggplot(index_portal_df, 
  ...
  annotate(
    "text",
    x = 0.65,
    y = means_portal_df$mean_value[means_portal_df$has_portal == "No"],
    label = paste0("Mean: ", round(means_portal_df$mean_value[means_portal_df$has_portal == "No"], 2)),
    size = 4,
    hjust = 1
  ) +
  ...
  annotate(
    "text",
    x = 1.35,
    y = means_portal_df$mean_value[means_portal_df$has_portal == "Yes"],
    label = paste0("Mean: ", round(means_portal_df$mean_value[means_portal_df$has_portal == "Yes"], 2)),
    size = 4,
    hjust = -0.8
  ) +
...
```{r}
#| label: export-continuous-indices-js

# Accessibility Index
accessibility_index_js <- accessibility_index %>%
  select(country, value = accessibility_index) %>%
  filter(!is.na(value))

js_content <- paste0(
  "const ACCESSIBILITY_INDEX_DATA = ",
  jsonlite::toJSON(accessibility_index_js, pretty = TRUE, auto_unbox = TRUE),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "accessibilityIndexData.js"))

# Comprehensiveness Index
comprehensiveness_index_js <- comprehensiveness_index %>%
  select(country, value = comprehensiveness_index) %>%
  filter(!is.na(value))

js_content <- paste0(
  "const COMPREHENSIVENESS_INDEX_DATA = ",
  jsonlite::toJSON(comprehensiveness_index_js, pretty = TRUE, auto_unbox = TRUE),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "comprehensivenessIndexData.js"))

# Overall Index
overall_index_js <- index_overall %>%
  select(country, value = index_overall) %>%
  filter(!is.na(value))

js_content <- paste0(
  "const OVERALL_INDEX_DATA = ",
  jsonlite::toJSON(overall_index_js, pretty = TRUE, auto_unbox = TRUE),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "overallIndexData.js"))

# Years Index
years_index_js <- years_coverage_df %>%
  select(country, value = years_index) %>%
  filter(!is.na(value))

js_content <- paste0(
  "const YEARS_INDEX_DATA = ",
  jsonlite::toJSON(years_index_js, pretty = TRUE, auto_unbox = TRUE),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "yearsIndexData.js"))

# Volume Index
volume_index_js <- volume %>%
  select(country, value = volume_index) %>%
  filter(!is.na(value))

js_content <- paste0(
  "const VOLUME_INDEX_DATA = ",
  jsonlite::toJSON(volume_index_js, pretty = TRUE, auto_unbox = TRUE),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "volumeIndexData.js"))

# COFOG Index
cofog_index_js <- cofog %>%
  select(country, value = cofog_index) %>%
  filter(!is.na(value))

js_content <- paste0(
  "const COFOG_INDEX_DATA = ",
  jsonlite::toJSON(cofog_index_js, pretty = TRUE, auto_unbox = TRUE),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "cofogIndexData.js"))
```

```{r}
#| label: export-all-indices-js

all_indices <- index_overall %>%
  select(country) %>%
  left_join(years_coverage_df %>% select(country, years_index), by = "country") %>%
  left_join(volume %>% select(country, volume_index), by = "country") %>%
  left_join(cofog %>% select(country, cofog_index), by = "country") %>%
  left_join(comprehensiveness_index %>% select(country, comprehensiveness_index), by = "country") %>%
  left_join(accessibility_index %>% select(country, accessibility_index), by = "country") %>%
  left_join(index_overall %>% select(country, overall_index = index_overall), by = "country")

all_indices_list <- split(all_indices[-1], all_indices$country)
all_indices_list <- lapply(all_indices_list, function(x) {
  as.list(x[1, ])
})

js_content <- paste0(
  "const ALL_INDICES_DATA = ",
  jsonlite::toJSON(all_indices_list, pretty = TRUE, auto_unbox = TRUE, na = "null"),
  ";\n"
)
writeLines(js_content, file.path(js_output_dir, "allIndicesData.js"))
```
