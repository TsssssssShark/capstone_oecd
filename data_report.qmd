...
ft_q34 <- comparison %>%
  ...
  bg(i = which(comparison$status == 'developed'), 
     bg = "#4682C8") %>% 
  bg(i = which(comparison$status == 'development'), 
     bg = "orange") %>%
  bg(i = which(comparison$status == 'removed'), 
     bg = "red") %>%
  bg(i = which(comparison$status == 'new_countries'), 
     bg = "#B19CD9") %>%
