...
# Horizontal bar chart
index_overall_chart <- ggplot(index_overall, 
                              aes(x = index_overall, 
                                  y = reorder(country_label,
                                    -index_overall, na.rm = TRUE))) +
  
  geom_bar(
    data = filter(index_overall, !is.na(index_overall)),
    stat = "identity", 
    fill = "#4682C8", 
    width = 0.6, 
    linewidth = 0.2
  ...
  geom_text(
    aes(x = ifelse(is.na(index_overall), 0.02, index_overall + 0.02),
        label = ifelse(is.na(index_overall), 
                       "NA", round(index_overall, 2)),
